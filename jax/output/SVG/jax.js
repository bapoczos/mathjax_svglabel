...
(function (AJAX,HUB,HTML,SVG) {
  ...
  SVG.Augment({
    ...
    preTranslate: function (state) {
      ...
      for (i = 0; i < m; i++) {
        script = scripts[i]; if (!script.parentNode) continue;
	      ...
	      console.log(script); //print out each latex 'script'
        //
	...
	if (HUB.Browser.noContextMenu) {
      ...
      for (i = 0; i < m; i++) {
        ...
        jax = script.MathJax.elementJax; if (!jax) continue;
	      //console.log(jax); crerate a jax for each latex script
    ...
    getJaxFromMath: function (math) {
      ...
      console.log(HUB.getJaxFor(math));
	...
	if (variant.h != null) {text.h = variant.h}; if (variant.d != null) {text.d = variant.d}
      ...
      for (i = 0, m = text.length; i < m; i++) {
        ...
        n = text.charCodeAt(i); c = text.charAt(i);
	      //console.log(c);  go through all character    
      ...
      svg.element.setAttribute("mypath","mypath"); //*** print out the path of this character ***
      return svg;
    },
    ...
    createDelimiter: function (code,HW,scale,font) {
      ...
      console.log(svg.element); //print out Delimiter
      return svg;
    },
    createChar: function (scale,data,font) {
      ...
      //console.log(text); //print out special characters
  ...
  var BBOX = SVG.BBOX = MathJax.Object.Subclass({
    ...
    Add: function (svg,dx,dy,forcew,infront) {
      ...
      if (svg.element) {
	      svg.element.setAttribute("my_element_data","my_element_data"); // Add a new element group, path,etc!!!
	  ...
	  if (nodeName === "g") {
	    svg.element.setAttribute("my_g_data","my_g_data"); //Add a new group
	    ...
	    svg.element.setAttribute("my_path_line_data","my_path_line_data"); //Add a path
  ...
  HUB.Register.StartupHook("mml Jax Ready",function () {
	...
	if (this.data[0] != null) {
	  ...
	  this.SVGhandleColor(svg);
        ...
        this.SVGsaveData(svg);
	svg.element.setAttribute('mylabel',this.labelname);
	return svg;
      }
    });
	...
	if (this.data[0] != null) {
	  ...
	  this.SVGhandleColor(svg);
        ...
        this.SVGsaveData(svg);
	svg.element.setAttribute('mystyle','mystyledata');
	return svg;
      },
      SVGstretchH: function (w) {
    ...
    MML.math.Augment({
	  ...
	  MML.mbase.prototype.displayIndent = HUB.config.displayIndent;
		...
		box.Clean();
          ...
          var svg = this.SVG();
	  svg.element.setAttribute("myData","Hello"); //each svg can get some extra info of needed
